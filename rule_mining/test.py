...
#
# # 方法一：在函数内部转换（已在函数中实现）
# normalized_table = normalize_columns(table)
#
# # 方法二：在调用函数时显式转换
# # normalized_table = normalize_columns(np.array(table))
#
...
a = np.array([0.4,0.6])
b = np.array([0,1])
print(euclidean_distance_squared(a, b))
...
#     small_ratio_threshold: float = 0.005
#     big_k: int = 3
#
#     @staticmethod
...
# # 调用方法获取可合并的列对
# mergeable_pairs = incorporator.get_mergeable_column_pairs(table)