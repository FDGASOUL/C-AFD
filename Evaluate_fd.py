...
def evaluate_fd(all_discovered_dependencies, ground_truth_path):
    ...
    # 新增误报和漏报详情输出
    if false_positive > 0:
        fp_details = "\n".join([f"{','.join(lhs)}->{rhs}" for lhs, rhs in discovered_set - groundtruth_set])
        logger.warning(f"发现误报FD（{false_positive}个）:\n{fp_details}")

    if false_negative > 0:
        fn_details = "\n".join([f"{','.join(lhs)}->{rhs}" for lhs, rhs in groundtruth_set - discovered_set])
        logger.warning(f"存在漏报FD（{false_negative}个）:\n{fn_details}")
